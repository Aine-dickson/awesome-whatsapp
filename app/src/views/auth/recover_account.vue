<template>
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <div class="w-full p-6 md:mt-0 sm:max-w-md sm:p-8">
            ...
            <form @submit.prevent="submit" @reset.prevent="$router.push({name: 'login'})" class="mt-4 space-y-4 lg:mt-5 md:space-y-5" action="#">
                <div>
                ...
                <button type="reset" class="w-full text-slate-900 border dark:text-white hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:hover:bg-blue-700 dark:focus:ring-blue-800">Discard</button>
            </form>
        </div>
    </div>
</template>
...
let submit = () => {
    ...
    accountStore.recover_account(email_address.value).then((response) => {
        if (response) {
